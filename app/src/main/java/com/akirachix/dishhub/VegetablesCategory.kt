...
class VegetablesCategory : AppCompatActivity() {
    ...
    private fun setupSaveButton() {
        binding.saveButton.setOnClickListener {
            if (selectedItems.isNotEmpty()) {
                saveSelectedItems(selectedItems)  // Call the function to save selected items
            } else {
                Toast.makeText(this, "No items selected", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun saveSelectedItems(it: Any) {
        selectedItems.forEach { vegetable ->
            val pantryItem = PantryItems(
                item = vegetable.name,
